...
def boost_user(row):
    b = 0
    if row["has_admin_inactive"]:                                   b += 30
    if row["is_orphaned"]:                                          b += 35
    if row["is_overprivileged"]:                                    b += 25
    if row["num_systems"] >= 6 and row["privilege_encoded"] <= 1:   b += 15
    # Suppression (but not for critical overrides)
    if row["is_new_hire"]:                                          b -= 40
...
# Score-based flag: users threshold=68, events=45
u_by_score = (users["risk_score"] >= 68)
e_by_score = (events["risk_score"] >= 45)

# Combine: (score OR hard_override) AND NOT exclusion
users["predicted_anomaly"]  = (
    (u_by_score | user_hard_override) & ~user_hard_exclusion_safe
).astype(int)
...
# Force severity for hard overrides
users.loc[u_override_stale_admin & ~user_hard_exclusion_safe, "severity"] = "CRITICAL"
users.loc[u_override_stale_admin & ~user_hard_exclusion_safe, "risk_score"] = \
    users.loc[u_override_stale_admin & ~user_hard_exclusion_safe, "risk_score"].clip(lower=85)

users.loc[u_override_orphaned & ~user_hard_exclusion_safe, "severity"] = "CRITICAL"
users.loc[u_override_orphaned & ~user_hard_exclusion_safe, "risk_score"] = \
    users.loc[u_override_orphaned & ~user_hard_exclusion_safe, "risk_score"].clip(lower=80)
